...
void CG_Player( centity_t *cent ) {
	...
	if(pug_demospec.integer!=-1 && !cg.renderingThirdPerson) {
...
// minimal_add:
	...
	if (cgs.gametype >= GT_TEAM && cg_drawFriend.integer &&
		cent->currentState.number != (pug_demospec.integer!=-1?pug_demospec.integer:cg.snap->ps.clientNum))			// Don't show a sprite above a player's own head in 3rd person.
