...
line 2297
;2294:
;2295:			//Holster their sabers now, until the duel starts (then they'll get auto-turned on to look cool)
;2296:
;2297:			if (!ent->client->ps.saberHolstered)
ADDRFP4 0
INDIRP4
CNSTI4 408
ADDP4
...
;2298:			{
line 2299
;2299:				G_Sound(ent, CHAN_AUTO, saberOffSound);
ADDRFP4 0
INDIRP4
ARGP4
CNSTI4 0
...
pop
line 2300
;2300:				ent->client->ps.weaponTime = 400;
ADDRFP4 0
INDIRP4
CNSTI4 408
ADDP4
...
ASGNI4
line 2301
;2301:				ent->client->ps.saberHolstered = qtrue;
ADDRFP4 0
INDIRP4
CNSTI4 408
ADDP4
...
LABELV $1155
line 2303
;2303:			if (!challenged->client->ps.saberHolstered)
ADDRLP4 1124
INDIRP4
CNSTI4 408
ADDP4
...
;2304:			{
line 2305
;2305:				G_Sound(challenged, CHAN_AUTO, saberOffSound);
ADDRLP4 1124
INDIRP4
ARGP4
CNSTI4 0
...
pop
line 2306
;2306:				challenged->client->ps.weaponTime = 400;
ADDRLP4 1124
INDIRP4
CNSTI4 408
ADDP4
...
ASGNI4
line 2307
;2307:				challenged->client->ps.saberHolstered = qtrue;
ADDRLP4 1124
INDIRP4
CNSTI4 408
ADDP4
...
;2311:		{
line 2313
;2312:			//Print the message that a player has been challenged in private, only announce the actual duel initiation in private
;2313:			trap_SendServerCommand( challenged-g_entities, va("cp \"%s %s\n\"", ent->client->pers.netname, G_GetStripEdString("SVINGAME", "PLDUELCHALLENGE")) );
ADDRGP4 $116
ARGP4
ADDRGP4 $1160
ARGP4
...
line 2317
;2316:
;2317:		challenged->client->ps.fd.privateDuelTime = 0; //reset the timer in case this player just got out of a duel. He should still be able to accept the challenge.
ADDRLP4 1124
INDIRP4
CNSTI4 408
ADDP4
...
line 2319
;2318:
;2319:		ent->client->ps.forceHandExtend = HANDEXTEND_DUELCHALLENGE;
ADDRFP4 0
INDIRP4
CNSTI4 408
ADDP4
...
ASGNI4
line 2320
;2320:		ent->client->ps.forceHandExtendTime = level.time + 1000;
ADDRFP4 0
INDIRP4
CNSTI4 408
ADDP4
...
line 2322
;2321:
;2322:		ent->client->ps.duelIndex = challenged->s.number;
ADDRFP4 0
INDIRP4
CNSTI4 408
ADDP4
...
ASGNI4
line 2323
;2323:		ent->client->ps.duelTime = level.time + 5000;
ADDRFP4 0
INDIRP4
CNSTI4 408
ADDP4
...
line 2410
;2326:
;2327:void PM_SetAnim(int setAnimParts,int anim,int setAnimFlags, int blendTime);
;2328:
;2329:#ifdef _DEBUG
;2330:extern stringID_table_t animTable[MAX_ANIMATIONS+1];
;2331:
;2332:void Cmd_DebugSetSaberMove_f(gentity_t *self)
;2333:{
;2334:	int argNum = trap_Argc();
;2335:	char arg[MAX_STRING_CHARS];
;2336:
;2337:	if (argNum < 2)
;2338:	{
;2339:		return;
;2340:	}
;2341:
;2342:	trap_Argv( 1, arg, sizeof( arg ) );
;2343:
;2344:	if (!arg[0])
;2345:	{
;2346:		return;
;2347:	}
;2348:
;2349:	self->client->ps.saberMove = atoi(arg);
;2350:	self->client->ps.saberBlocked = BLOCKED_BOUNCE_MOVE;
;2351:
;2352:	if (self->client->ps.saberMove >= LS_MOVE_MAX)
;2353:	{
;2354:		self->client->ps.saberMove = LS_MOVE_MAX-1;
;2355:	}
;2356:
;2357:	Com_Printf("Anim for move: %s\n", animTable[saberMoveData[self->client->ps.saberMove].animToUse].name);
;2358:}
;2359:
;2360:void Cmd_DebugSetBodyAnim_f(gentity_t *self, int flags)
;2361:{
;2362:	int argNum = trap_Argc();
;2363:	char arg[MAX_STRING_CHARS];
;2364:	int i = 0;
;2365:	pmove_t pmv;
;2366:
;2367:	if (argNum < 2)
...
;2370:	}
;2371:
;2372:	trap_Argv( 1, arg, sizeof( arg ) );
;2373:
;2374:	if (!arg[0])
;2375:	{
;2376:		return;
;2377:	}
;2378:
;2379:	while (i < MAX_ANIMATIONS)
;2380:	{
;2381:		if (!Q_stricmp(arg, animTable[i].name))
;2382:		{
;2383:			break;
;2384:		}
;2385:		i++;
;2386:	}
;2387:
;2388:	if (i == MAX_ANIMATIONS)
;2389:	{
;2390:		Com_Printf("Animation '%s' does not exist\n", arg);
;2391:		return;
;2392:	}
;2393:
;2394:	memset (&pmv, 0, sizeof(pmv));
;2395:	pmv.ps = &self->client->ps;
;2396:	pmv.animations = bgGlobalAnimations;
;2397:	pmv.cmd = self->client->pers.cmd;
;2398:	pmv.trace = trap_Trace;
;2399:	pmv.pointcontents = trap_PointContents;
;2400:	pmv.gametype = g_gametype.integer;
;2401:
;2402:	pm = &pmv;
;2403:	PM_SetAnim(SETANIM_BOTH, i, flags, 0);
;2404:
;2405:	Com_Printf("Set body anim to %s\n", arg);
;2406:}
;2407:#endif
;2408:
;2409:void StandardSetBodyAnim(gentity_t *self, int anim, int flags)
;2410:{
line 2413
;2411:	pmove_t pmv;
;2412:
;2413:	memset (&pmv, 0, sizeof(pmv));
ADDRLP4 0
ARGP4
CNSTI4 0
ARGI4
...
ASGNP4
line 2415
;2415:	pmv.animations = bgGlobalAnimations;
ADDRLP4 0+224
ADDRGP4 bgGlobalAnimations
ASGNP4
line 2416
;2416:	pmv.cmd = self->client->pers.cmd;
ADDRLP4 0+4
ADDRFP4 0
INDIRP4
CNSTI4 408
...
ASGNB 28
line 2417
;2417:	pmv.trace = trap_Trace;
ADDRLP4 0+240
ADDRGP4 trap_Trace
ASGNP4
line 2418
;2418:	pmv.pointcontents = trap_PointContents;
ADDRLP4 0+244
ADDRGP4 trap_PointContents
ASGNP4
line 2419
;2419:	pmv.gametype = g_gametype.integer;
ADDRLP4 0+220
ADDRGP4 g_gametype+12
INDIRI4
ASGNI4
...
ASGNP4
line 2422
;2422:	PM_SetAnim(SETANIM_BOTH, anim, flags, 0);
CNSTI4 3
ARGI4
ADDRFP4 4
INDIRI4
...
line 2436
;2424:
;2425:void DismembermentTest(gentity_t *self);
;2426:
;2427:#ifdef _DEBUG
;2428:void DismembermentByNum(gentity_t *self, int num);
;2429:#endif
;2430:
;2431:/*
;2432:=================
...
;2434:=================
;2435:*/
;2436:void ClientCommand( int clientNum ) {
line 2440
;2437:	gentity_t *ent;
;2438:	char	cmd[MAX_TOKEN_CHARS];
;2439:
;2440:	ent = g_entities + clientNum;
ADDRLP4 1024
CNSTI4 828
ADDRFP4 0
INDIRI4
...
NEU4 $1172
line 2442
;2442:		return;		// not fully in game yet
ADDRGP4 $1171
JUMPV
LABELV $1172
line 2446
...
;2444:
;2445:
;2446:	trap_Argv( 0, cmd, sizeof( cmd ) );
CNSTI4 0
ARGI4
ADDRLP4 0
ARGP4
...
line 2449
;2447:
;2448:	//rww - redirect bot commands
;2449:	if (strstr(cmd, "bot_") && AcceptBotCommand(cmd, ent))
ADDRLP4 0
ARGP4
ADDRGP4 $1176
ARGP4
...
NEI4 $1177
line 2456
;2456:		Cmd_Say_f (ent, SAY_ALL, qfalse);
ADDRLP4 1024
INDIRP4
ARGP4
ADDRLP4 1040
...
line 2459
;2458:	}
;2459:	if (Q_stricmp (cmd, "say_team") == 0) {
ADDRLP4 0
ARGP4
ADDRGP4 $1182
ARGP4
...
NEI4 $1180
line 2460
;2460:		Cmd_Say_f (ent, SAY_TEAM, qfalse);
ADDRLP4 1024
INDIRP4
ARGP4
CNSTI4 1
...
NEI4 $1186
line 2468
;2468:		Cmd_Voice_f (ent, SAY_ALL, qfalse, qfalse);
ADDRLP4 1024
INDIRP4
ARGP4
ADDRLP4 1052
...
line 2471
;2470:	}
;2471:	if (Q_stricmp (cmd, "vsay_team") == 0) {
ADDRLP4 0
ARGP4
ADDRGP4 $1191
ARGP4
...
NEI4 $1189
line 2472
;2472:		Cmd_Voice_f (ent, SAY_TEAM, qfalse, qfalse);
ADDRLP4 1024
INDIRP4
ARGP4
CNSTI4 1
...
NEI4 $1192
line 2476
;2476:		Cmd_VoiceTell_f ( ent, qfalse );
ADDRLP4 1024
INDIRP4
ARGP4
CNSTI4 0
...
NEI4 $1194
line 2480
;2480:		Cmd_Voice_f (ent, SAY_ALL, qfalse, qtrue);
ADDRLP4 1024
INDIRP4
ARGP4
ADDRLP4 1064
...
line 2483
;2482:	}
;2483:	if (Q_stricmp (cmd, "vosay_team") == 0) {
ADDRLP4 0
ARGP4
ADDRGP4 $1199
ARGP4
...
NEI4 $1197
line 2484
;2484:		Cmd_Voice_f (ent, SAY_TEAM, qfalse, qtrue);
ADDRLP4 1024
INDIRP4
ARGP4
ADDRLP4 1068
...
line 2487
;2486:	}
;2487:	if (Q_stricmp (cmd, "votell") == 0) {
ADDRLP4 0
ARGP4
ADDRGP4 $1202
ARGP4
...
NEI4 $1200
line 2488
;2488:		Cmd_VoiceTell_f ( ent, qtrue );
ADDRLP4 1024
INDIRP4
ARGP4
CNSTI4 1
...
line 2491
;2490:	}
;2491:	if (Q_stricmp (cmd, "vtaunt") == 0) {
ADDRLP4 0
ARGP4
ADDRGP4 $1205
ARGP4
...
;2498:	}
;2499:
;2500:	// ignore all other commands when at intermission
;2501:	if (level.intermissiontime)
ADDRGP4 level+9008
INDIRI4
CNSTI4 0
EQI4 $1208
...
;2502:	{
line 2503
;2503:		qboolean giveError = qfalse;
ADDRLP4 1080
CNSTI4 0
ASGNI4
line 2505
;2504:
;2505:		if (!Q_stricmp(cmd, "give"))
ADDRLP4 0
ARGP4
ADDRGP4 $1213
ARGP4
...
LABELV $1211
line 2509
;2509:		else if (!Q_stricmp(cmd, "god"))
ADDRLP4 0
ARGP4
ADDRGP4 $1216
ARGP4
...
LABELV $1214
line 2513
;2513:		else if (!Q_stricmp(cmd, "notarget"))
ADDRLP4 0
ARGP4
ADDRGP4 $1219
ARGP4
...
LABELV $1217
line 2517
;2517:		else if (!Q_stricmp(cmd, "noclip"))
ADDRLP4 0
ARGP4
ADDRGP4 $1222
ARGP4
...
LABELV $1220
line 2521
;2521:		else if (!Q_stricmp(cmd, "kill"))
ADDRLP4 0
ARGP4
ADDRGP4 $1225
ARGP4
...
LABELV $1223
line 2525
;2525:		else if (!Q_stricmp(cmd, "teamtask"))
ADDRLP4 0
ARGP4
ADDRGP4 $267
ARGP4
...
LABELV $1226
line 2529
;2529:		else if (!Q_stricmp(cmd, "levelshot"))
ADDRLP4 0
ARGP4
ADDRGP4 $1230
ARGP4
...
LABELV $1228
line 2533
;2533:		else if (!Q_stricmp(cmd, "follow"))
ADDRLP4 0
ARGP4
ADDRGP4 $1233
ARGP4
...
LABELV $1231
line 2537
;2537:		else if (!Q_stricmp(cmd, "follownext"))
ADDRLP4 0
ARGP4
ADDRGP4 $1236
ARGP4
...
LABELV $1234
line 2541
;2541:		else if (!Q_stricmp(cmd, "followprev"))
ADDRLP4 0
ARGP4
ADDRGP4 $1239
ARGP4
...
LABELV $1237
line 2545
;2545:		else if (!Q_stricmp(cmd, "team"))
ADDRLP4 0
ARGP4
ADDRGP4 $1242
ARGP4
...
LABELV $1240
line 2549
;2549:		else if (!Q_stricmp(cmd, "forcechanged"))
ADDRLP4 0
ARGP4
ADDRGP4 $1245
ARGP4
...
NEI4 $1243
line 2550
;2550:		{ //special case: still update force change
line 2551
;2551:			Cmd_ForceChanged_f (ent);
ADDRLP4 1024
INDIRP4
ARGP4
...
line 2554
;2553:		}
;2554:		else if (!Q_stricmp(cmd, "where"))
ADDRLP4 0
ARGP4
ADDRGP4 $1248
ARGP4
...
LABELV $1246
line 2558
;2558:		else if (!Q_stricmp(cmd, "callvote"))
ADDRLP4 0
ARGP4
ADDRGP4 $1251
ARGP4
...
LABELV $1249
line 2562
;2562:		else if (!Q_stricmp(cmd, "vote"))
ADDRLP4 0
ARGP4
ADDRGP4 $1254
ARGP4
...
LABELV $1252
line 2566
;2566:		else if (!Q_stricmp(cmd, "callteamvote"))
ADDRLP4 0
ARGP4
ADDRGP4 $1257
ARGP4
...
LABELV $1255
line 2570
;2570:		else if (!Q_stricmp(cmd, "teamvote"))
ADDRLP4 0
ARGP4
ADDRGP4 $1260
ARGP4
...
LABELV $1261
line 2578
;2578:		else if (!Q_stricmp(cmd, "setviewpos"))
ADDRLP4 0
ARGP4
ADDRGP4 $1266
ARGP4
...
LABELV $1264
line 2582
;2582:		else if (!Q_stricmp(cmd, "stats"))
ADDRLP4 0
ARGP4
ADDRGP4 $1269
ARGP4
...
;2588:		{
line 2589
;2589:			trap_SendServerCommand( clientNum, va("print \"You cannot perform this task (%s) during the intermission.\n\"", cmd ) );
ADDRGP4 $1272
ARGP4
ADDRLP4 0
ARGP4
...
;2592:		{
line 2593
;2593:			Cmd_Say_f (ent, qfalse, qtrue);
ADDRLP4 1024
INDIRP4
ARGP4
CNSTI4 0
...
LABELV $1273
line 2602
;2602:	else if (Q_stricmp (cmd, "god") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1216
ARGP4
...
LABELV $1275
line 2604
;2604:	else if (Q_stricmp (cmd, "notarget") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1219
ARGP4
...
LABELV $1277
line 2606
;2606:	else if (Q_stricmp (cmd, "noclip") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1222
ARGP4
...
LABELV $1279
line 2608
;2608:	else if (Q_stricmp (cmd, "kill") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1225
ARGP4
...
LABELV $1281
line 2610
;2610:	else if (Q_stricmp (cmd, "teamtask") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $267
ARGP4
...
LABELV $1283
line 2612
;2612:	else if (Q_stricmp (cmd, "levelshot") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1230
ARGP4
...
LABELV $1285
line 2614
;2614:	else if (Q_stricmp (cmd, "follow") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1233
ARGP4
...
LABELV $1287
line 2616
;2616:	else if (Q_stricmp (cmd, "follownext") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1236
ARGP4
...
LABELV $1289
line 2618
;2618:	else if (Q_stricmp (cmd, "followprev") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1239
ARGP4
...
LABELV $1291
line 2620
;2620:	else if (Q_stricmp (cmd, "team") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1242
ARGP4
...
LABELV $1293
line 2622
;2622:	else if (Q_stricmp (cmd, "forcechanged") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1245
ARGP4
...
LABELV $1295
line 2624
;2624:	else if (Q_stricmp (cmd, "where") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1248
ARGP4
...
LABELV $1297
line 2626
;2626:	else if (Q_stricmp (cmd, "callvote") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1251
ARGP4
...
LABELV $1299
line 2628
;2628:	else if (Q_stricmp (cmd, "vote") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1254
ARGP4
...
LABELV $1301
line 2630
;2630:	else if (Q_stricmp (cmd, "callteamvote") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1257
ARGP4
...
LABELV $1303
line 2632
;2632:	else if (Q_stricmp (cmd, "teamvote") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1260
ARGP4
...
LABELV $1305
line 2634
;2634:	else if (Q_stricmp (cmd, "gc") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1263
ARGP4
...
LABELV $1307
line 2636
;2636:	else if (Q_stricmp (cmd, "setviewpos") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1266
ARGP4
...
LABELV $1309
line 2638
;2638:	else if (Q_stricmp (cmd, "stats") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1269
ARGP4
...
line 2725
;2640:	/*
;2641:	else if (Q_stricmp(cmd, "#mm") == 0 && CheatsOk( ent ))
;2642:	{
;2643:		G_PlayerBecomeATST(ent);
;2644:	}
;2645:	*/
;2646:	//I broke the ATST when I restructured it to use a single global anim set for all client animation.
;2647:	//You can fix it, but you'll have to implement unique animations (per character) again.
;2648:#ifdef _DEBUG //sigh..
;2649:	else if (Q_stricmp(cmd, "headexplodey") == 0 && CheatsOk( ent ))
;2650:	{
;2651:		Cmd_Kill_f (ent);
;2652:		if (ent->health < 1)
;2653:		{
;2654:			float presaveVel = ent->client->ps.velocity[2];
;2655:			ent->client->ps.velocity[2] = 500;
;2656:			DismembermentTest(ent);
;2657:			ent->client->ps.velocity[2] = presaveVel;
;2658:		}
;2659:	}
;2660:	else if (Q_stricmp(cmd, "g2animent") == 0 && CheatsOk( ent ))
;2661:	{
;2662:		G_CreateExampleAnimEnt(ent);
;2663:	}
;2664:	else if (Q_stricmp(cmd, "loveandpeace") == 0 && CheatsOk( ent ))
;2665:	{
;2666:		trace_t tr;
;2667:		vec3_t fPos;
;2668:
;2669:		AngleVectors(ent->client->ps.viewangles, fPos, 0, 0);
;2670:
;2671:		fPos[0] = ent->client->ps.origin[0] + fPos[0]*40;
;2672:		fPos[1] = ent->client->ps.origin[1] + fPos[1]*40;
;2673:		fPos[2] = ent->client->ps.origin[2] + fPos[2]*40;
;2674:
;2675:		trap_Trace(&tr, ent->client->ps.origin, 0, 0, fPos, ent->s.number, ent->clipmask);
;2676:
;2677:		if (tr.entityNum < MAX_CLIENTS && tr.entityNum != ent->s.number)
;2678:		{
;2679:			gentity_t *other = &g_entities[tr.entityNum];
;2680:
;2681:			if (other && other->inuse && other->client)
;2682:			{
;2683:				vec3_t entDir;
;2684:				vec3_t otherDir;
;2685:				vec3_t entAngles;
;2686:				vec3_t otherAngles;
;2687:
;2688:				if (ent->client->ps.weapon == WP_SABER && !ent->client->ps.saberHolstered)
;2689:				{
;2690:					Cmd_ToggleSaber_f(ent);
;2691:				}
;2692:
;2693:				if (other->client->ps.weapon == WP_SABER && !other->client->ps.saberHolstered)
;2694:				{
;2695:					Cmd_ToggleSaber_f(other);
;2696:				}
;2697:
;2698:				if ((ent->client->ps.weapon != WP_SABER || ent->client->ps.saberHolstered) &&
;2699:					(other->client->ps.weapon != WP_SABER || other->client->ps.saberHolstered))
;2700:				{
;2701:					VectorSubtract( other->client->ps.origin, ent->client->ps.origin, otherDir );
;2702:					VectorCopy( ent->client->ps.viewangles, entAngles );
;2703:					entAngles[YAW] = vectoyaw( otherDir );
;2704:					SetClientViewAngle( ent, entAngles );
;2705:
;2706:					StandardSetBodyAnim(ent, BOTH_KISSER1LOOP, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_HOLDLESS);
;2707:					ent->client->ps.saberMove = LS_NONE;
;2708:					ent->client->ps.saberBlocked = 0;
;2709:					ent->client->ps.saberBlocking = 0;
;2710:
;2711:					VectorSubtract( ent->client->ps.origin, other->client->ps.origin, entDir );
;2712:					VectorCopy( other->client->ps.viewangles, otherAngles );
;2713:					otherAngles[YAW] = vectoyaw( entDir );
;2714:					SetClientViewAngle( other, otherAngles );
;2715:
;2716:					StandardSetBodyAnim(other, BOTH_KISSEE1LOOP, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_HOLDLESS);
;2717:					other->client->ps.saberMove = LS_NONE;
;2718:					other->client->ps.saberBlocked = 0;
;2719:					other->client->ps.saberBlocking = 0;
;2720:				}
;2721:			}
;2722:		}
;2723:	}
;2724:#endif
;2725:	else if (Q_stricmp(cmd, "thedestroyer") == 0 && CheatsOk( ent ) && ent && ent->client && ent->client->ps.saberHolstered && ent->client->ps.weapon == WP_SABER)
ADDRLP4 0
ARGP4
ADDRGP4 $1315
ARGP4
...
line 2729
;2728:
;2729:		if (!ent->client->ps.saberHolstered)
ADDRLP4 1024
INDIRP4
CNSTI4 408
ADDP4
...
;2730:		{
line 2731
;2731:			if (ent->client->ps.dualBlade)
ADDRLP4 1024
INDIRP4
CNSTI4 408
ADDP4
...
;2732:			{
line 2733
;2733:				ent->client->ps.dualBlade = qfalse;
ADDRLP4 1024
INDIRP4
CNSTI4 408
ADDP4
...
ASGNI4
line 2735
;2734:				//ent->client->ps.fd.saberAnimLevel = FORCE_LEVEL_1;
;2735:			}
ADDRGP4 $1314
JUMPV
LABELV $1318
...
;2737:			{
line 2738
;2738:				ent->client->ps.dualBlade = qtrue;
ADDRLP4 1024
INDIRP4
CNSTI4 408
ADDP4
...
line 2740
;2739:
;2740:				trap_SendServerCommand( -1, va("print \"%sTHE DESTROYER COMETH\n\"", S_COLOR_RED) );
ADDRGP4 $1320
ARGP4
ADDRGP4 $1321
ARGP4
...
pop
line 2741
;2741:				G_ScreenShake(vec3_origin, NULL, 10.0f, 800, qtrue);
ADDRGP4 vec3_origin
ARGP4
CNSTP4 0
ARGP4
...
pop
line 2743
;2742:				//ent->client->ps.fd.saberAnimLevel = FORCE_LEVEL_3;
;2743:			}
line 2744
;2744:		}
line 2745
...
line 2794
;2746:#ifdef _DEBUG
;2747:	else if (Q_stricmp(cmd, "debugSetSaberMove") == 0)
;2748:	{
;2749:		Cmd_DebugSetSaberMove_f(ent);
;2750:	}
;2751:	else if (Q_stricmp(cmd, "debugSetBodyAnim") == 0)
;2752:	{
;2753:		Cmd_DebugSetBodyAnim_f(ent, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
;2754:	}
;2755:	else if (Q_stricmp(cmd, "debugDismemberment") == 0)
;2756:	{
;2757:		Cmd_Kill_f (ent);
;2758:		if (ent->health < 1)
;2759:		{
;2760:			char	arg[MAX_STRING_CHARS];
;2761:			int		iArg = 0;
;2762:
;2763:			if (trap_Argc() > 1)
;2764:			{
;2765:				trap_Argv( 1, arg, sizeof( arg ) );
;2766:
;2767:				if (arg[0])
;2768:				{
;2769:					iArg = atoi(arg);
...
;2771:			}
;2772:
;2773:			DismembermentByNum(ent, iArg);
;2774:		}
;2775:	}
;2776:	else if (Q_stricmp(cmd, "debugKnockMeDown") == 0)
;2777:	{
;2778:		ent->client->ps.forceHandExtend = HANDEXTEND_KNOCKDOWN;
;2779:		ent->client->ps.forceDodgeAnim = 0;
;2780:		if (trap_Argc() > 1)
;2781:		{
;2782:			ent->client->ps.forceHandExtendTime = level.time + 1100;
;2783:			ent->client->ps.quickerGetup = qfalse;
;2784:		}
;2785:		else
;2786:		{
;2787:			ent->client->ps.forceHandExtendTime = level.time + 700;
;2788:			ent->client->ps.quickerGetup = qtrue;
;2789:		}
;2790:	}
;2791:#endif
;2792:
...
;2794:	{
line 2795
;2795:		if (Q_stricmp(cmd, "addbot") == 0)
ADDRLP4 0
ARGP4
ADDRGP4 $1324
ARGP4
...
NEI4 $1322
line 2796
;2796:		{ //because addbot isn't a recognized command unless you're the server, but it is in the menus regardless
line 2798
;2797://			trap_SendServerCommand( clientNum, va("print \"You can only add bots as the server.\n\"" ) );
;2798:			trap_SendServerCommand( clientNum, va("print \"%s.\n\"", G_GetStripEdString("SVINGAME", "ONLY_ADD_BOTS_AS_SERVER")));
ADDRGP4 $116
ARGP4
ADDRGP4 $1326
ARGP4
...
;2801:		{
line 2802
;2802:			trap_SendServerCommand( clientNum, va("print \"unknown cmd %s\n\"", cmd ) );
ADDRGP4 $1327
ARGP4
ADDRLP4 0
ARGP4
